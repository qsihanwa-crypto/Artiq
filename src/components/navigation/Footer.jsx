...
const SOCIAL_ICON = { Facebook, YouTube: Youtube }

export default function Footer() {
  return (
    <footer className="relative overflow-hidden border-t border-neutral-200 bg-white pt-24 text-zinc-950">
      ...
      <div className="mx-auto mt-24 max-w-6xl border-t border-neutral-200 px-6 py-16 sm:px-10">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-[1.4fr_1fr_1fr]">
          <div>
            <h3 className="footer-name select-none font-display text-6xl font-semibold tracking-tight text-zinc-950 transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] hover:-skew-x-2 hover:scale-[1.02] sm:text-7xl">
              {site.artistName}
            </h3>
            <p className="mt-4 max-w-sm text-zinc-600">{site.tagline}</p>
              ...
              {site.social.map((s) => {
                ...
                return (
                  <a
                    ...
                    className="flex h-11 w-11 items-center justify-center rounded-full border border-neutral-200 text-zinc-950 transition-colors hover:bg-black/5"
                  >
          ...
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Explore</h4>
            <ul className="mt-4 flex flex-col gap-3">
              {NAV_LINKS.map((link) => (
                <li key={link.to}>
                  <NavLink to={link.to} className="text-zinc-700 underline-offset-4 hover:text-zinc-950 hover:underline">
                    {link.label}
                  </NavLink>
                </li>
              ))}
          ...
          <div>
            <h4 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Follow along</h4>
            <ul className="mt-4 flex flex-col gap-3">
              <li className="text-zinc-700">
                Watch the process and behind-the-scenes on{' '}
                <a
