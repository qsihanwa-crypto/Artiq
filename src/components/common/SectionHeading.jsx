...
export default function SectionHeading({ kicker, title, subtitle, align = 'left', className = '' }) {
  ...
  return (
    <div className={`flex flex-col gap-5 ${alignClass} ${className}`}>
      {kicker && (
        <span className="text-xs font-semibold uppercase tracking-[0.2em] text-neutral-500">{kicker}</span>
      )}
      ...
      {subtitle && <p className="max-w-xl text-lg leading-relaxed text-neutral-600">{subtitle}</p>}
    </div>
  )
}
