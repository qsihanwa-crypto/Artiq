...
/*
  InfiniteSpiral — React Bits component, JS-CSS variant.
  Source:   https://reactbits.dev/components/infinite-spiral
  Registry: https://reactbits.dev/r/InfiniteSpiral-JS-CSS.json

  Vendored from the registry with upstream code style kept intact for easy
  diffing against future versions. Project-specific seams, all additive:

    - `linkComponent` prop: when an item has `to`, the card renders through
      this component (this app passes react-router's <Link>) so clicking a
      thumbnail is a client-side navigation, not a full reload.
    - `data-cursor="view"` on interactive cards, to pick up the site cursor.
    - onFocusCapture / onBlurCapture on the root reuse the hover-pause path,
      so a keyboard user tabbing through cards isn't chasing a moving target.

  Reduced motion: upstream already zeroes the auto-spin under
  `prefers-reduced-motion`; the Home section additionally drops to
  `animationMode="drag"` so nothing moves until the visitor asks it to. The
  plain, always-available path is the "View full catalogue" button beside it.
*/
    ...
    >
      <div className="infinite-spiral__stage" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          const interactive = Boolean(item.to || item.href);
          const Card = item.to ? LinkComponent : item.href ? 'a' : 'div';
          return (
            <Card
